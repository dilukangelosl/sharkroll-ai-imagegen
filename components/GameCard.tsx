...
const GameCard: React.FC<GameCardProps> = ({ game, status, onRetry }) => {
  const isProcessing = ['downloading', 'generating', 'compositing'].includes(status.status);
    ...
    <div className="w-[140px] h-[188px] shrink-0 bg-slate-800 border border-slate-700 rounded-lg overflow-hidden flex flex-col group relative shadow-md hover:shadow-xl transition-shadow">
      ...
      <div className="w-full h-full relative bg-slate-900">
        ...
        {status.status === 'complete' && status.finalImageBase64 ? (
          ...
          />
        ) : status.status === 'idle' || status.status === 'downloading' || status.status === 'error' ? (
           ...
           status.cleanedImageBase64 ? (
             <img src={status.cleanedImageBase64} className="w-full h-full object-cover opacity-50" />
           ) : (
        ...
        <div className="absolute inset-x-0 bottom-0 p-2 bg-gradient-to-t from-black/95 via-black/60 to-transparent z-10">
            <h3 className="font-bold text-[11px] leading-tight text-white truncate drop-shadow-md" title={game.name}>{game.name}</h3>
            <p className="text-[9px] text-slate-300 truncate drop-shadow-md">{game.providerName}</p>
        </div>
        ...
        {status.status === 'error' && (
          <div className="absolute inset-0 bg-red-900/90 flex flex-col items-center justify-center p-2 text-center z-20">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-red-200 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <p className="text-[9px] text-red-100 mb-2 leading-tight line-clamp-2">{status.error || "Failed"}</p>
            <button 
              onClick={onRetry}
